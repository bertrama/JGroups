...
/**
 * NIO based client for measuring heap-based vs direct byte buffers. Use {@link NioServerTest} as server
 * @author Bela Ban
 * @since  3.6.4
 */
public class NioClientTest {
    ...
    protected class Sender extends Thread {
        ...
        public Sender(InetAddress host, boolean direct, CountDownLatch latch) {
            ...
            this.host=host;
            this.direct=direct;
            buf=create(NioServerTest.SIZE, direct);
        }
        ...
        public void run() {
            ...
            for(;;) {
                if(total_bytes_sent.addAndGet(NioServerTest.SIZE) > NioServerTest.BYTES_TO_SEND)
                    break;
                buf.rewind();
                try {
                    ch.write(buf);
